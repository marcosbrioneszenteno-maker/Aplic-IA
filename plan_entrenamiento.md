# Plan Completo de Entrenamiento y Nutrición

## Perfil: Ectomorfo avanzado en regreso, 24 años, 1.75m, 67-70kg

---

## DIAGNÓSTICO RÁPIDO

### Lo positivo
- **Banca 90kg x1 fácil a 70kg de peso corporal = 1.28x BW**. Eso es MUY bueno para estar "volviendo". Tu fuerza de empuje se mantuvo e incluso mejoró (antes 80kg 1RM a los 17). El upper body responde bien.
- Base multideportiva excepcional: coordinación, propiocepción, capacidad de aprendizaje motor.
- Ya tomás suplementos inteligentes (magnesio glicinato, lion's mane, omega 3, creatina).

### Lo que hay que corregir ya
1. **Comés poco e inconsistente.** Si tu mantenimiento es 2900 y a veces comés 2700, estás en déficit. NO VAS A CRECER ASÍ. Este es tu cuello de botella #1.
2. **La espalda baja.** Dos lesiones en peso muerto + molestia crónica = hay que programar inteligentemente alrededor de esto. Nada de peso muerto convencional por ahora.
3. **Parones.** 5 meses con interrupciones. La consistencia mata al talento inconsistente.
4. **Se te acabó la creatina.** Comprá ya. Es el suplemento más importante para vos.

---

## PLAN DE ENTRENAMIENTO: Upper/Lower (Fuerza) + PPL (Hipertrofia)

### Filosofía del plan
- **Lunes-Martes**: Upper/Lower de FUERZA - compuestos pesados, pocas reps, pocos ejercicios
- **Miércoles-Jueves-Viernes**: PPL de HIPERTROFIA - más volumen, más reps, más aislamiento
- **Cada sesión 45-60 min MÁXIMO** - si no te alcanza el tiempo, algo está mal
- **Máximo 6 ejercicios por sesión** - así los hacés TODOS y no recortás por fatiga
- **Lo importante va primero** - los accesorios que siempre salteabas ahora están integrados inteligentemente

### Estructura semanal

| Día | Sesión | Enfoque | Duración |
|-----|--------|---------|----------|
| **Lunes** | Upper FUERZA | Compuestos pesados 3x5 | 45-50 min |
| **Martes** | Lower FUERZA | Sentadilla pesada + accesorios | 45-50 min |
| **Miércoles** | Push HIPERTROFIA | Pecho + hombros + tríceps | 50-60 min |
| **Jueves** | Pull HIPERTROFIA | Espalda + bíceps + traps | 50-60 min |
| **Viernes** | Legs HIPERTROFIA | Cuádriceps + isquios + pantorrillas | 50-60 min |

---

### LUNES - Upper FUERZA (50 min)

Filosofía: pesado, compuesto, pocas series, descansos largos + laterales para frecuencia extra.

| # | Ejercicio | Series x Reps | RPE | Descanso | Notas |
|---|-----------|---------------|-----|----------|-------|
| 1 | Press banca plano con barra | 3x5 | 8 | 3 min | Progresión: +2.5kg cuando las 3x5 se sientan sólidas. |
| 2 | Remo mancuerna 1 brazo | 3x5 | 8 | 2-3 min | Apoyado en banco. Cero estrés lumbar. Pesado. |
| 3 | Press militar SENTADO | 3x5 | 8 | 2-3 min | SENTADO para proteger lumbar. |
| 4 | Dominadas con lastre | 3x5 | 8 | 2-3 min | Espalda ancha. Lastre progresivo. |
| 5 | Fondos en paralelas con lastre | 3x5 | 8 | 2 min | Pecho + tríceps pesado. |
| 6 | Elevaciones laterales rápidas | 3x15-20 | 8 | 45 seg | Frecuencia extra de hombro lateral. Poco descanso, liviano, pump. |

**6 ejercicios, 18 series, 50 min. Las laterales al final son livianas y rápidas, no agregan fatiga real pero sí volumen al deltoides lateral.**

---

### MARTES - Lower FUERZA (45-50 min)

**CERO peso muerto convencional. La sentadilla es tu lift principal de pierna.**

| # | Ejercicio | Series x Reps | RPE | Descanso | Notas |
|---|-----------|---------------|-----|----------|-------|
| 1 | Sentadilla con barra | 3x5 | 8 | 3 min | Ejercicio principal. Profundidad cómoda para tu lumbar. |
| 2 | RDL con mancuernas | 3x8 | 7 | 2-3 min | LIVIANO. Patrón de bisagra seguro. Si molesta: hip thrust. |
| 3 | Curl femoral sentado o acostado | 3x10 | 8 | 90 seg | **ISQUIOS - NO TE LO SALTÉS.** Va tercero para que llegues fresco. |
| 4 | Pantorrilla de pie | 3x12 | 8 | 60 seg | Rango completo, pausa abajo y arriba. |
| 5 | Bird dogs + Dead bugs | 2x8/lado cada uno | - | 60 seg | Prehab lumbar. 4 minutos y listo. |

**5 ejercicios, ~14 series, 45 min.** Los isquios van TERCEROS, no últimos. Así llegás con energía y los hacés siempre.

---

### MIÉRCOLES - Push HIPERTROFIA (50-60 min)

| # | Ejercicio | Series x Reps | RPE | Descanso | Notas |
|---|-----------|---------------|-----|----------|-------|
| 1 | Press inclinado mancuernas | 3x8-12 | 8 | 2 min | **PECHO SUPERIOR = prioridad visual.** Ángulo 30-45°. |
| 2 | Elevaciones laterales máquina o mancuernas | 4x12-15 | 8-9 | 60 seg | **HOMBRO LATERAL = prioridad #1 visual.** Va segundo, no último. |
| 3 | Crossover en poleas o pec deck | 3x12-15 | 8-9 | 60 seg | Pecho. Stretch abajo, squeeze arriba. |
| 4 | Press plano mancuernas | 3x10-12 | 8 | 90 seg | Pecho medio. |
| 5 | Extensión de tríceps con soga | 3x12-15 | 8-9 | 60 seg | Separar la soga abajo. |
| 6 | Extensión de tríceps sobre cabeza (copa) | 3x10-12 | 8-9 | 60 seg | Cabeza larga del tríceps = brazo grande de costado. |

**6 ejercicios, 19 series, 55 min.** Laterales van SEGUNDAS para que no las recortes nunca.

---

### JUEVES - Pull HIPERTROFIA (55-60 min)

| # | Ejercicio | Series x Reps | RPE | Descanso | Notas |
|---|-----------|---------------|-----|----------|-------|
| 1 | Jalón al pecho agarre amplio | 4x8-12 | 8 | 2 min | **DORSALES ANCHOS = prioridad visual.** Agarre ancho, llevar al pecho, squeeze. |
| 2 | Remo con mancuerna 1 brazo | 3x10-12 | 8 | 90 seg | Apoyado en banco = cero estrés lumbar. Espalda gruesa. |
| 3 | Encogimientos trapecios mancuernas | 4x10-12 | 8-9 | 60 seg | **TRAPECIOS = prioridad visual.** Subieron a 4 series. Van terceros, no al final. |
| 4 | Face pulls | 3x15-20 | 8 | 60 seg | Rear delts + salud de hombros. |
| 5 | Curl bíceps barra EZ | 3x10-12 | 8-9 | 60 seg | Pico de bíceps. |
| 6 | Curl martillo | 3x10-12 | 8 | 60 seg | Braquial = brazo grueso de costado. |

**6 ejercicios, 20 series, 55 min.** Dorsales 4 series, trapecios 4 series. Lo que se ve.

---

### VIERNES - Legs HIPERTROFIA + Laterales (55-60 min)

| # | Ejercicio | Series x Reps | RPE | Descanso | Notas |
|---|-----------|---------------|-----|----------|-------|
| 1 | Hack squat o prensa | 3x8-12 | 8 | 2 min | Cuádriceps. NO sentadilla trasera pesada (ya fue el martes). |
| 2 | Curl femoral sentado o acostado | 3x10-12 | 8-9 | 90 seg | **ISQUIOS DE NUEVO. Segunda frecuencia semanal.** |
| 3 | Hip thrust | 3x10-12 | 8 | 90 seg | Glúteos. Pausa arriba 2 seg. |
| 4 | Extensiones de cuádriceps | 3x12-15 | 8-9 | 60 seg | Contracción arriba. |
| 5 | Elevaciones laterales (polea o máquina) | 4x12-15 | 8-9 | 60 seg | **3ra frecuencia semanal de hombro lateral. Clave.** |
| 6 | Pantorrilla sentado | 3x15-20 | 8 | 60 seg | Sóleo. Rango completo. |
| 7 | Bird dogs + Plancha | 2x8/lado + 2x30seg | - | 60 seg | Prehab lumbar. 4 min. |

**7 ejercicios, ~21 series, 55 min.** Isquios van SEGUNDOS. Laterales 3ra frecuencia semanal.

---

## RESUMEN DE VOLUMEN SEMANAL POR GRUPO (OPTIMIZADO PARA IMPACTO VISUAL)

| Grupo muscular | Series/semana | Frecuencia | Prioridad visual |
|---------------|---------------|------------|-----------------|
| **Hombro lateral** | **11** (lun 3 + mié 4 + vie 4) | **3x** | **#1 - MÁXIMA** |
| **Espalda (dorsal)** | **13** (lun 3 + jue 7 + mar indirecto) | **2x** | **#2 - MÁXIMA** |
| **Trapecios** | **4** (jue 4) | **1x** | **#3 - ALTA** |
| **Brazos total** | **12** (jue 6 bíceps + mié 6 tríceps) | **1-2x** | **#4 - ALTA** |
| **Pecho** | **12** (lun 3 + mié 9) | **2x** | **#5 - ALTA** |
| Hombro frontal | 6 (lun 3 + mié indirecto) | 2x | Cubierto con presses |
| Rear delts | 3 (jue 3) | 1x | Ok con face pulls |
| Cuádriceps | 9 (mar 3 + vie 6) | 2x | Base del rugby |
| Isquiotibiales | 9 (mar 6 + vie 3) | 2x | Protección lumbar |
| Glúteos | 3-6 (mar indirecto + vie 3) | 2x | Ok |
| Pantorrillas | 6 (mar 3 + vie 3) | 2x | Ok |
| Core/Prehab | Integrado en mar y vie | 4x | Protección lumbar |

---

## PROGRESIÓN (Mesociclo de 5 semanas)

### Días de FUERZA (Lunes/Martes)
| Semana | Series | Peso | RPE |
|--------|--------|------|-----|
| 1 | 3x5 como está escrito | Peso cómodo | 7 |
| 2 | 3x5 | +2.5kg en compuestos principales | 7-8 |
| 3 | 3x5 | +2.5kg más si la semana 2 fue bien | 8 |
| 4 | 3x5 | Intentar mantener o +2.5kg | 8-9 |
| 5 | **DELOAD**: 3x5 con -20% de peso | 5-6 |

### Días de HIPERTROFIA (Miércoles/Jueves/Viernes)
| Semana | Volumen | Estrategia | RPE |
|--------|---------|------------|-----|
| 1 | Como está escrito | Encontrar pesos, sentir el músculo | 7 |
| 2 | Mismo volumen | Subir 1-2 reps donde se pueda | 7-8 |
| 3 | +1 serie en ejercicio 1 y 2 de cada día | Pico de volumen | 8 |
| 4 | Mantener volumen de semana 3 | Semana más dura | 8-9 |
| 5 | **DELOAD**: quitar 1 serie de cada ejercicio, -15% peso | 5-6 |

Después de semana 5: nuevo mesociclo con pesos base más altos.

---

## PLAN DE NUTRICIÓN: 3200-3300 KCAL

### Macros objetivo diarios

| Macro | Gramos | kcal | % |
|-------|--------|------|---|
| **Proteína** | 150g (2.1g/kg) | 600 | 18% |
| **Carbohidratos** | 440g | 1760 | 54% |
| **Grasas** | 80g | 720 | 22% |
| **Total** | - | **~3100-3200 kcal** | 100% |

**Por qué 3200:** Tu mantenimiento es 2900 → superávit de 300-400 kcal = **lean bulk**. Ganancia esperada: 0.25-0.5kg por semana. Si en 2 semanas no subís, sumamos 200 kcal más.

---

### Distribución en 5 comidas (no 3, no 4, CINCO)

#### Comida 1 - Desayuno (7-8am) ~700 kcal
- 80g avena con 250ml leche
- 2 huevos enteros revueltos
- 1 banana
- 10g mantequilla de maní en la avena

#### Comida 2 - Media mañana (10-11am) ~500 kcal
- 150g pollo o carne a la plancha
- 150g arroz blanco cocido
- Verduras al gusto
- 1 cucharada aceite de oliva

#### Comida 3 - Almuerzo (1-2pm) ~700 kcal
- 180g carne roja magra o pescado
- 200g arroz o pasta cocida
- Ensalada grande
- 1 cucharada aceite de oliva
- 1 fruta

#### Comida 4 - Pre o post entreno (5-6pm) ~600 kcal
- **BATIDO HIPERCALÓRICO CASERO** (reemplaza el mass gainer):
  - 300ml leche
  - 1 scoop whey protein (30g)
  - 60g avena
  - 1 banana
  - 20g mantequilla de maní
  - *Opcional: 5g creatina acá*

#### Comida 5 - Cena (8-9pm) ~600 kcal
- 150g pollo, pescado o huevos (3-4 huevos)
- 150g batata/boniato o papa
- Verduras salteadas con aceite de oliva
- 30g frutos secos (almendras, nueces)

---

### Trucos para comer más sin sufrir

1. **El batido es tu arma secreta.** 600 kcal líquidas son más fáciles que una comida sólida. Tomalo, no lo discutas.
2. **Aceite de oliva en todo.** 1 cucharada = 120 kcal. 2 por día = 240 kcal extra sin sentirlo.
3. **Frutos secos entre comidas.** Un puñado de 30g = 180 kcal. Llevá en el bolsillo.
4. **Arroz blanco > integral.** Más fácil de digerir, menos saciedad, podés comer más.
5. **Si no tenés hambre, comé igual.** El reloj manda, no el estómago. En volumen se come por disciplina.
6. **Dejá de comer "tan limpio".** No te digo que comas basura, pero pasta, arroz blanco, pan, banana, jugo natural, miel... TODO SUMA y no es "sucio".

---

### Sobre el mass gainer

**Te recomiendo dejarlo y hacerte el batido casero.** El mass gainer comercial suele ser maltodextrina (azúcar barata) cara. El batido casero es mejor, más barato y con ingredientes reales. Si querés seguir usándolo mientras tenés, está bien, pero empezá a transicionar.

---

## SUPLEMENTACIÓN

| Suplemento | Dosis | Cuándo | Prioridad |
|------------|-------|--------|-----------|
| **Creatina monohidratada** | 5g/día todos los días | Con cualquier comida | **URGENTE - COMPRÁ YA** |
| **Whey protein** | 1 scoop (25-30g) | En el batido hipercalórico | Alta |
| **Omega 3** | 2-3g EPA+DHA | Con comida | **COMPRÁ - antiinflamatorio para tu lumbar** |
| **Magnesio glicinato** | 400mg | Antes de dormir | Alta (sueño + recuperación) |
| **Lion's mane** | Seguir dosis actual | Mañana | Media (cognitivo) |
| **Multivitamínico** | 1/día | Con comida | Media |
| **Citrulina malato** | 6-8g | 30 min pre-entreno | Baja (opcional, nice to have) |
| **Vitamina D3** | 2000-4000 UI | Con comida con grasa | Alta si no tomás sol seguido |

---

## PROTOCOLO LUMBAR - ESTO ES CLAVE

### Lo que NO hacés
- ❌ Peso muerto convencional con barra
- ❌ Remo con barra inclinado pesado (reemplazá por remo en máquina con soporte de pecho)
- ❌ Buenos días con barra
- ❌ Sentadilla si duele la lumbar ese día (cambiá a hack squat o prensa)
- ❌ Hiperextensiones con peso

### Lo que SÍ hacés TODOS LOS DÍAS de entrenamiento
- ✅ **Bird dogs**: 2x10 por lado (activación glúteo-lumbar segura)
- ✅ **Dead bugs**: 2x10 por lado (core profundo sin cargar la espalda)
- ✅ **Plancha**: 2-3x30-60 seg (estabilidad)
- ✅ **Pallof press**: 2x10 por lado (anti-rotación)
- ✅ **Cat-cow**: 10 reps antes de entrenar (movilidad lumbar)
- ✅ **RDL con mancuernas LIGERO**: si no duele, es mejor que peso muerto convencional

### Si la lumbar molesta un día
1. Cambiá sentadilla → hack squat o prensa
2. Cambiá RDL → curl femoral + hip thrust
3. Cambiá remo con barra → remo en máquina con soporte de pecho
4. Hacé más prehab ese día

---

## CHECKPOINTS Y MÉTRICAS

### Pesaje
- Pesarte TODOS LOS DÍAS en ayunas, después de ir al baño
- Calcular PROMEDIO SEMANAL (no mirar día a día, fluctúa 1-2kg)
- Objetivo: subir 0.25-0.5kg de promedio semanal

### Cada 2 semanas
- Fotos de progreso: frente relajado, frente en tensión, lateral, espalda
- Misma luz, misma hora, mismo lugar
- Medidas con cinta: pecho, brazos (contraído), cintura, muslos

### Cada 4 semanas
- Revisar pesos del gimnasio: ¿subieron?
- Revisar peso corporal promedio: ¿subió 1-2kg en el mes?
- Si no subió: +200 kcal (un puñado más de frutos secos + más arroz)
- Ajustar el plan

---

## RESUMEN: TUS 5 REGLAS NO NEGOCIABLES

1. **COMÉ 3200 KCAL TODOS LOS DÍAS.** No hay excusas. El batido hipercalórico te salva el día.
2. **COMPRÁ CREATINA Y OMEGA 3 ESTA SEMANA.** La creatina es lo más importante que podés tomar como ectomorfo.
3. **CERO PESO MUERTO CONVENCIONAL.** RDL con mancuernas es tu nuevo mejor amigo.
4. **HACÉ PREHAB LUMBAR TODOS LOS DÍAS.** Bird dogs + dead bugs + plancha. 5 minutos que te salvan meses de dolor.
5. **NO FALTES.** Lunes a viernes sin parones. La consistencia es literalmente todo.
